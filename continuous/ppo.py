...
from continuous.models import ActorCritic
...
class PPO:
    ...
    def update(self, memory):
        ...
        old_states = torch.squeeze(
            torch.stack(memory.states).to(self.device), 1).detach()
        old_actions = torch.squeeze(
            torch.stack(memory.actions).to(self.device), 1).detach()
